...
6. **Access notes and customer contacts stay local.** `units.access_notes` (gate codes, job-box combos, site-super cell, dogs) must **never** be sent to ZenSched: not in `location_create` `notes`, not in `event_create` `notes` or `title`, not in a form, not in a `shift_cancel` reason. Customer names, contact names, phones, and emails also stay in SQLite: ZenSched location names and event titles use the **street address** (`"<street>, <city>"`), never the customer or contact name. Tell the driver access details in person or by a channel the owner chooses. If the owner asks you to put a code or a name in ZenSched, decline and explain why.
7. **Always pass an `idempotency_key` to every mutating ZenSched call**, using the exact formats below.
8. **Always use the business's local timezone offset** from `settings.timezone_offset` in `shift_create` `start` / `end` (e.g. `2026-09-07T07:00:00-05:00`). Never send `Z`. The `units_due` view computes `start_iso` and `end_iso` for you from that setting, so **when the clocks change** (US DST ends the first Sunday of November and starts the second Sunday of March) update it first: `UPDATE settings SET value = '-06:00' WHERE key = 'timezone_offset';` for Central standard time. Before scheduling any week that crosses a DST change, check the offset the view will emit for those dates; a shift sent with the old offset lands an hour off.
...
3. For each **new** place (no cache hit): `location_create(name="<street>, <city>", street_address="<full address>", checkin_radius_m=75, idempotency_key="loc-unit-{unit_id}")`, e.g. `name="8800 FM 1826, Austin"`. Metered $0.03 (rule 11). **Do not put access notes in `notes`, and do not put the customer's name in `name`** (rule 6); the customer stays in SQLite. `checkin_radius_m` here is informational; widen with `policy_update` (rule 13). If `pin_quality` is `street` that is fine for a house; for a construction yard or a festival field, offer `location_update(location_id, lat, lng)` (free) or `location_refine` ($0.10) only if the owner reports missed check-ins.
...
7. Confirm: "Added Harbor Builders, two toilets (T-14, T-15) at 8800 FM 1826, weekly $22 each, next service Mon Sep 7. Gate code and the Harbor contact are saved locally only; ZenSched just has the address and pin. Same GPS pin for both — I did not geocode twice."
...
2. `event_create(location_id=<zensched_location_id>, title="Portables - <street>", start_date=window_start, end_date=window_end, idempotency_key="event-loc-{zensched_location_id}-{window_start as YYYYMMDD}")`. No access notes and no customer name in `title` or `notes` (rule 6).
...
7. The trigger advances `next_service_date` only when `serviced = 'Yes'`. Do not update it yourself. If this was a one-off extra pump on a weekly unit and the owner wants the regular stop kept, set that weekly row's `next_service_date` back to what it was. If `serviced = 'No'`, the date stays put — ask whether to re-attempt or skip to the next regular date, then `UPDATE service_schedule SET next_service_date = ?` to the agreed date. Never leave a past `next_service_date` in place: `units_due` would emit the **same** `shift-schedule-{schedule_id}-{YYYYMMDD}` key as the failed stop, and ZenSched would replay that already-checked-out shift instead of creating a new one. A re-attempt on the same calendar day needs the `-2` suffix.
